...
require 'json'
file = File.read('./db/answers.json')
answers_data = JSON.parse(file)

for data in answers_data do
    evaluation = Evaluations.create!(semester: data['semester'], name: data['name'])
    subject = Subject.create!(code: data['code'], name: data['name'])
    class_information = Cclass.create!(code: data['classCode'], subject: subject)
    Answer.create!(evaluation: evaluation, cclass: class_information, subject: subject, code:['code'])
end
